...
predmat = zeros(model.n_cla, n, n_kernel); % w
isSV = zeros(1,n);
...
for epoch = 1:model.T
    ...
    for i = 1:n
        ...
        val_f = preds(:, :)*weights'; % w*\phi, means margins
        
        yi = Y(idxs_subgrad);
        
        margin_true = val_f(yi); %margin of y_i
        val_f(yi) = -Inf;
        [margin_pred, yhat] = max(val_f); %margin of y_u, y_u
        
        model.errTot = model.errTot+(margin_true<=margin_pred);
        model.lossTot = model.lossTot+max(1-margin_true+margin_pred,0);
        
        %elastic net
        lr = model.lambda*model.iter; %t*\lambda
        ...
        if margin_true<1+margin_pred
            ...
            sqnorms = sqnorms+2*(preds(yi, :)-preds(yhat, :))+(2*Kii(:))'; % update theta(2,2)
            norms = sqrt(sqnorms); % 2-form of theta
            trunc_norms = max(norms-model.iter*model.alpha-model.gamma*model.localR,0); % v
            norm_trunc_theta = norm(trunc_norms+eps,model.q); % q-form of v
