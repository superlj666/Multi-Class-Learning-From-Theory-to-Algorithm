...
%    where $K$ and $n$ are the number of classes and size of the sample, respectively.
    ...
    multi-class kernel learning methods based on local Rademacher complexity are sound and effective.
...
implemented by stochastic sub-gradient descent by updating dual weight.
...
\textbf{VC-dimension Bounds.}
    ...
    order to characterize multi-class PAC learn ability,
...
A common approach to multi-class classification is the use of feature
...
%$\left[\sum_{i=1}^K\|\mathbf w_i\|_2^p\right]^{\frac{1}{p}}$ equals to $\left[\sum_{i=1}^M\|\mathbf w_i\|_2^p\right]^{\frac{1}{p}}$
%when $p=2$
...
local Rademancher complexity (the tail sum of the eigenvalues of the kernel) leads to
...
\max\limits_{i \not= j}\left|1-\left(\langle\mathbf w,
\phi_{\bm \mu}(\mathbf{x}_i)\rangle -\langle\mathbf w, \phi_{\bm \mu}(\mathbf{x}_j)\rangle\right)\right|_+,$$
and
...
seen in Algorithm \ref{algorithm2}.
 ...
 and then $\pmb{\mu}$ is updated together with $\mathbf w$ according to a tricky link function given in Theorem \ref{theorem-fenchel-dual}.
...
The algorithm initializes $q=2\log K$ to make the order of generalization reach
...
Actually, the algorithm only updates scalar values $\|\bm \theta_m^{t+1}\|$, $\nu_m^{t+1}$ and $\mu_m^{t+1}$,
avoiding high-dimensional calculation among $\mathbf w^{t+1}$ and $\bm \theta_m^{t+1}$.
...
%&=\|\pmb{\theta}_m^{t}\|_2^2-2\pmb{\theta}_m^t \cdot (\phi_m(\mathbf{x})-\phi_m(\mathbf{x}'))
%+ 2\kappa_m(\mathbf{x}, \mathbf{x})^2
\end{align*}
where $z^t=\partial\ell(\mathbf w^t,\phi_{\bm \mu}(\mathbf{x}^t), y^t)$.
% and $\bm \mu$ is used to chosen the maximum predict label $\mathbf{x}'$.
...
\begin{align*}
    ...
    =\frac{\mathrm{sgn}({c}_m^\ast)|{c}_m^\ast|^{q-1}}{\|\pmb{c}^\ast\|_q^{q-2}}.
\end{align*}
Let
$
    \bm \nu=\Big[\|\pmb{\theta}_1\|-\beta r_1,\ldots,
...
Convergence rate of the proposed \texttt{Conv-MKL} is depended on which $\ell_p$ MC-MKL algorithm it uses.
In experiments, we implement \texttt{Conv-MKL} based on UFO-MKL \cite{OrabonaL11} in next section.
Thus, convergence rate of \texttt{Conv-MKL} is same as convergence rate of UFO-MKL
analysed in Section 4.1 of \cite{OrabonaL11}.
...
we now state the convergence theorem for any loss function that satisfies the following hypothesis
\begin{align}
\label{loss_function_hypothesis}
    \|z_m\| \leq L\|\phi_m(\mathbf{w}, \cdot)\|_2, \forall m=1,\ldots, M, y' \in \mathcal{Y}.
\end{align}
...
\begin{align*}
    f(\mathbf w^{T+1})-f(\mathbf w^\ast) \leq \frac{eL^2(1+\log T)}{2\alpha T},
\end{align*}
...
\begin{align*}
    &\|\partial\ell(\mathbf w^t,\phi(\mathbf{x}^t,\cdot), y^t)\|_{2,q} \\
    &\leq LM^{1/q} \max_{j=1, \ldots, M} \|\phi_m(\mathbf{x}^t, \cdot)\|_2 \leq LM^{1/q}
\end{align*}
The function $\Omega(\mathbf w)=\frac{\alpha}{2}\|\mathbf w\|_{2,p}^2 + \beta\mathbf{\mu} \cdot \mathbf{r}$ is
 $\alpha$-strongly convex w.r.t. the norm $\|\cdot\|_{2,q}$. Hence, according to Theorem 1 in \cite{Shalev-ShwartzSS07},
...
the Multi-class MKL (MC-MKL) for $\ell_1$-norm and $\ell_2$-norm \cite{ZienO2007}
...
the DOGMA library \footnote{Available at http://dogma. sourceforge. net} (LMC, GMNP,
$\ell_1$-norm and $\ell_2$-norm MC-MKL) and the SHOGUN-6.1.3 \footnote{Available at http://www.shogun-toolbox.org/} (UFO-MKL).
What's more, we implement our proposed \texttt{Conv-MKL} and \texttt{SMSD-MKL} algorithms based on UFO-MKL.

We experiment on 14 publicly available data sets:
...
where $\tau \in {2^i, i=-10,-9,\ldots,9,10}$. For single Kernel methods (One vs. One, One vs. Rest and GMNP),
...
2) Winning on 9 data sets and defeated on 3 data sets, \texttt{SMSD-MKL} is significantly better than \texttt{Conv-MKL};
