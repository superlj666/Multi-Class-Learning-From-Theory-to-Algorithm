...
%    where $K$ and $n$ are the number of classes and size of the sample, respectively.
    ...
    generalization error bound with convergence rate of $\mathcal{O}\big(\frac{(\log K)^{2+{1}/{\log K}}}{n}\big)$,
    ...
    with statistical guarantees and fast convergence rate.
    ...
    local Rademacher complexity based multi-class kernel learning methods are sound and effective.
...
algorithms, which have statistic guarantees and fast convergence rate.
...
The global Rademacher complexity of $\mathcal{H}_{mkl}$ can be bounded by the trace of kernel matrix $\mathbf{K}_\mu$.
...
the local Rademancher complexity (the tail sum of the eigenvalues of the kernel) lead to
tighter generalization bounds than the global Rademancher complexity (the trace).
Thus, we add the local Rademancher complexity to restrict $\mathcal{H}_{mkl}$:
\begin{align}
    \mathcal{H}_{1}=\Big\{h_{\mathbf{w}, k_\mu} \in \mathcal{H}_{mkl}:\sum_{j > \zeta} \lambda(\mathbf{K}_\mu) \leq 1\Big\},
\end{align}
where $\mathbf{K}_\mu=\sum_{m=1}^M \mu_m \mathbf{K}_m$,
$\lambda_j(\mathbf K_{\bm \mu})$ is the $j$-th eigenvalues of $\mathbf K_{\bm \mu}$,
$\zeta$ is free parameter removing the $\zeta$ largest eigenvalues to control the tail sum.
Note that the tail sum is the difference between the trace and
the $\theta$ largest eigenvalues: $\sum_{j>\zeta}\lambda_j(\mathbf{K})=\mathrm{Tr}(\mathbf{K})-\sum_{j=1}^\zeta\lambda_j(\mathbf{K})$,
...
\begin{align*}
    \begin{aligned}
        ...
        & =\sum_{m=1}^M\mu_m/\|\mathbf{\mu}\|_1 \sum_{j>\zeta}\lambda_{j}(\|\mathbf{\mu}\|_1\mathbf{K}_m)\\
        & \leq \sum_{j>\zeta} \lambda_j\big(\mathbf{K}_\mu\big).
    \end{aligned}
\end{align*}
...
according to $\tilde{\kappa}_m:=\left(\sum_{j>\zeta}\lambda_j(\kappa_m)\right)^{-1}\kappa$
...
\begin{align*}
  \mathcal{H}_{2}=\Big\{&f_{\mathbf{w},\tilde{\kappa}_{\bm \mu}} \in \mathcal{H}_{mkl}:\\
    &\sum_{m=1}^M\mu_m\sum_{j>\zeta}\lambda_{j}(\mathbf{K}_m) \leq 1,\bm \mu\succeq 0,\|\bm \mu\|_1 \leq 1\Big\}.
\end{align*}
   ...
   \label{algorithm1}
    \begin{algorithmic}
       ...
       \FOR{$i=1$ {\bfseries to} $M$}
        \STATE Compute tail sum: $t_m=\sum_{j>\zeta}\mathbf{K}_m$
...
\max\limits_{i \not= j}\left|1-\left(\langle\mathbf w_i, \phi_{\bm \mu}(\mathbf{x}_i\rangle -\langle\mathbf w_j, \phi_{\bm \mu}(\mathbf{x}_j\rangle\right)\right|_+,$$
and
...
\begin{algorithm}[t]
   ...
   \label{algorithm2}
    \begin{algorithmic}
       \STATE {\bfseries Input:} $\alpha, \beta, \zeta, \bm \mu, T$
       \STATE {\bfseries Initialize:} $\mathbf w^1=\mathbf{0}, \pmb{\theta}^1=\mathbf{0}, \bm \mu^1=\mathbf{1}, q=2\log M $
       ...
       {\alpha\|\bm \theta_m^{t+1}\||\nu_m^{t+1}|_q^{q-2}}$, $\forall m=1,\ldots, M.$
       %\STATE $\mathbf w^{t+1}_m=\frac{\text{sgn}(\bm \nu_m) \pmb{\theta}_m^{t+1}}{t\alpha\|\pmb{\theta}_m^{t+1}\|}
       %\frac{|\bm \nu_m|^{q-1}}{\|\bm \nu\|_q^{q-2}}$, $\forall m=1,\ldots, M.$
       %\STATE $\mu_m=\frac{\mathbf w^{t+1}_m}{\pmb{\theta}^{t+1}_m}$, $\forall m=1,\ldots, M.$
       \ENDFOR
    \end{algorithmic}
\end{algorithm}
...
For $C(\mathbf w)$, the algorithm randomly samples a training example from train set at each iteration.
The algorithm updates the dual vector with the gradient of $C(\mathbf w)$. Since hinge loss used in $C(\mathbf w)$ is not differentiable,
the algorithm uses sub-gradient of $z^t=\partial\ell(\mathbf w^t,\phi_{\bm \mu}(\mathbf{x}_t), y_t)$ instead,
where $\partial\ell(\mathbf w^t,\phi_{\bm \mu}(\mathbf{x}_t), y_t)$ is the sub-gradient w.r.t $\mathbf w^t$.
...
The algorithm initializes $q=2\log M$ and $M>2$, thus $p=1/(1-1/q) \in (1,2)$, with this choice of $p$
the regularization becomes similar to the entropic regularization \cite{OrabonaFCC10}.

Actually, the algorithm updates real numbers $\|\bm \theta_m^{t+1}\|$, $\nu_m^{t+1}$ and $\mu_m^{t+1}$ in scalar products instead of
updating high-dimensional variables $\mathbf w^{t+1}$ and $\bm \theta_m^{t+1}$.
The $\|\bm \theta_m^{t+1}\|$ can be calculated in an efficient incremental way as following:
\begin{align*}
\|\pmb{\theta}_m^{t+1}\|
...
To reach the above argmax, the derivative of argmax should be zero, so $\mathbf w$ must be proportional to $\pmb{\theta}$. As in UFO-MKL \cite{OrabonaL11}, we explicitly give a tricky link function $\mathbf w_m=\pmb{\mu}_m\pmb{\theta}_m$ for different kernels. By this explicit link function, the algorithm can update both $\mathbf w$ and $\pmb{\mu}$ by $\nabla\Omega^\ast(\pmb{\theta})$. Then, for the convenience of computation, we focus on $\pmb{c}_m=\pmb{\mu}_m\|\pmb{\theta}_m\|$, rewriting the argmax:
\begin{align}
...
$
and use $\pmb{\mu}_m=\pmb{c}_m/\|\pmb{\theta}_m\|$ and $\mathbf w_m=\pmb{\mu}_m\pmb{\theta}_m$, We can get
\begin{align*}
...
Meanwhile, use all basic kernels for MKL methods (SMSD-MKL, conv, $\ell_1$ MC-MKL, $\ell_2$ MC-MKL, UFO-MKL).
...
Then, we design a two algorithms with statistical guarantees and fast convergence rates:
...
Empirical results show our methods outperform the state-of-the-art multiclass classification methods.
Our analysis based on local Rademacher complexity could be used as the basis for the
...
%\renewcommand{\captionfont}{\small}
   \caption{
    \small Comparison of average test accuracies of our \texttt{conv} and \texttt{SMSD-MKL} with the others including
    the linear multi-class SVM (LSVM),
    One-against-One (One vs. One),
    One-against-the-Rest(One vs. Rest),
    Generalized Minimal Norm Problem solver (GMNP),
...
we choose one kernel which have the highest performance among basic kernels by cross-validation.
Meanwhile, use all basic kernels for MKL methods (SMSD-MKL, conv, $\ell_1$ MC-MKL, $\ell_2$ MC-MKL, UFO-MKL).
For our algorithms \texttt{conv} and \texttt{SMSD-MKL}, we fix the parameter $\theta=2$ to curve the local Rademancher complexity.
The regularization parameterized $C \in {2^i, i=-2, \ldots, 12}$ of all algorithms,
the parameter $\gamma \in {10^i, i=-4, \ldots, 1}$ of \texttt{SMSD-MKL} are determined by 5-folds cross-validation on training data.
The other parameters for the compared algorithms follow the same experimental setting in their papers.
For each data set, we run all methods 50 times with randomly selected 80\% for training and 20\% for testing.
...
The average test accuracies are reported in Table \ref{tabel:accuracy}.
The results show: 1) Our \texttt{SMSD-MKL} and \texttt{Conv-MKL} methods lead to a consistent improvement over all data sets except \textit{vehicle};
2) Our \texttt{SMSD-MKL} and \texttt{Conv-MKL} methods is significantly
better than the compared Multiple Kernel Learning methods ($\ell_1$ MC-MKL,
$\ell_2$ MC-MKL and UFO-MKL) over all data sets;
3) \texttt{SMSD-MKL}  is significantly better than state-of-the-art \texttt{conv} on most data sets;
4) The multiple kernel methods usually better than the compared single kernel methods (One vs. One, One vs. Rest, GMNP);
