...
\end{align}
where $\mathbf{K}_{\bm \mu}=\sum_{m=1}^M \mu_m \mathbf{K}_m$,
...
according to $\tilde{\mathbf K}_m:=\left(\sum_{j>\zeta}\lambda_j(\mathbf K_m)\right)^{-1}\mathbf K_{\bm \mu}$
   ...
   \label{algorithm1}
    \begin{algorithmic}
       ...
       \FOR{$i=1$ {\bfseries to} $M$}
        \STATE Compute tail sum: $r_m=\sum_{j>\zeta}\lambda\left(\mathbf{K}_m\right)$
