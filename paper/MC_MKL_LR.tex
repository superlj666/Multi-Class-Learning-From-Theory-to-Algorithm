...
%In this paper,
%we study the generalization performance of multi-class classification based on the notion of local Rademacher complexity,
%and obtain--for the first time--a data-depend generalization error bound with convergence rates
%$\mathcal{O}\Big(\frac{(\log K)^{2+{1}/{\log K}}}{n}\Big)$,
%substantially improving the state-of-the-art bounds in the existing data-dependent
...
where
\begin{align*}
    %&\;\forall i:\xi_i = \langle\mathbf{w},\phi(\mathbf{x}_i,y_i)\rangle-
    %\max_{u \not= y_i}\langle\mathbf{w},\phi(\mathbf{x}_i,u)\rangle \\
