...
\begin{algorithm}[t]
   ...
   \label{algorithm2}
    \begin{algorithmic}
       ...
       \STATE {\bfseries Initialize:} $\pmb{w}^1=\mathbf{0}, \pmb{\theta}^1=\mathbf{0}, \mathbf{\mu}^1=\mathbf{1}, q=2\log M $
       ...
       {\alpha\|\pmb{\theta}_m\|\|\pmb{v}_m\|_q^{q-2}}$, $\forall m=1,\ldots, M.$
       %\STATE $\pmb{w}^{t+1}_m=\frac{\text{sgn}(\pmb{v}_m) \pmb{\theta}_m^{t+1}}{t\alpha\|\pmb{\theta}_m^{t+1}\|}
       %\frac{|\pmb{v}_m|^{q-1}}{\|\pmb{v}\|_q^{q-2}}$, $\forall m=1,\ldots, M.$
       %\STATE $\mu_m=\frac{\pmb{w}^{t+1}_m}{\pmb{\theta}^{t+1}_m}$, $\forall m=1,\ldots, M.$
       \ENDFOR
    \end{algorithmic}
\end{algorithm}
...
Based on the Stochastic Mirror Descent framework for minimization in \cite{Shalev-ShwartzT11, OrabonaL11},
 ...
 %The connection between the two vectors is via a link function $\pmb{\theta}=f(\pmb{w})$, where $f:\mathcal{K}\times\mathcal{M}\times\mathcal{H} \to \mathcal{K}\times\mathcal{M}\times\mathcal{H}$. Function $\Omega$ is strictly convex, so the algorithm use $\pmb{\theta}=f(\pmb{w})=\nabla\Omega(\pmb{w})$ as link function and $\pmb{w}==\nabla\Omega^\ast(\pmb{\theta})$ as the inverse function.Similar with UFO-MKL in \cite{OrabonaL11},
